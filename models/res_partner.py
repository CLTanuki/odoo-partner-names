...
class ResPartner(models.Model):
    ...
    @api.model
    def _get_computed_name(self, first_name, last_name, patronymic):
        """Compute the 'name' field according to splitted data.
        You can override this method to change the order of lastname and
        firstname the computed name"""
        order = self._get_names_order()
        if order == 'last_first_mid':
            return " ".join((p for p in (last_name, first_name, patronymic) if p))
        else:
            return " ".join((p for p in (last_name, patronymic, first_name) if p))
    ...
    def _compute_name(self):
        ...
        for record in self:
            record.name = record._get_computed_name(
                record.first_name, record.last_name, record.patronymic
            )
